...
typedef struct _sdl3_joypad
{
   SDL_Joystick   *joypad;
   SDL_Gamepad    *gamepad;
   SDL_JoystickID  jid;       /* 0 = slot unused */
   unsigned        num_axes;
...
static sdl3_joypad_t sdl3_pads[MAX_USERS];

static const char *sdl_joypad_name(unsigned pad)
{
   if (pad >= MAX_USERS || !sdl3_pads[pad].jid)
      return NULL;

   if (sdl3_pads[pad].gamepad)
      return SDL_GetGamepadName(sdl3_pads[pad].gamepad);
   return SDL_GetJoystickName(sdl3_pads[pad].joypad);
}

static uint8_t sdl3_pad_get_button(sdl3_joypad_t *pad, unsigned button)
{
   if (pad->gamepad)
...
static uint8_t sdl3_pad_get_hat(sdl3_joypad_t *pad, unsigned hat)
{
   /* Gamepads always have num_hats=0; this is only reached for raw joysticks. */
   return SDL_GetJoystickHat(pad->joypad, hat);
}

static int16_t sdl3_pad_get_axis(sdl3_joypad_t *pad, unsigned axis)
{
   if (pad->gamepad)
      return SDL_GetGamepadAxis(pad->gamepad, (SDL_GamepadAxis)axis);
   return SDL_GetJoystickAxis(pad->joypad, axis);
}

static void sdl3_pad_connect(SDL_JoystickID jid)
{
   int i;
   int slot             = -1;
   int32_t vendor       = 0;
...
static void sdl3_pad_disconnect(SDL_JoystickID jid)
{
   int i;

   for (i = 0; i < MAX_USERS; i++)
   {
      if (sdl3_pads[i].jid != jid)
         continue;

      if (sdl3_pads[i].gamepad)
      {
         SDL_CloseGamepad(sdl3_pads[i].gamepad);
         input_autoconfigure_disconnect(i, sdl_joypad.ident);
      }
      else if (sdl3_pads[i].joypad)
      {
         SDL_CloseJoystick(sdl3_pads[i].joypad);
         input_autoconfigure_disconnect(i, sdl_joypad.ident);
      }

      memset(&sdl3_pads[i], 0, sizeof(sdl3_pads[i]));
      return;
   }
}
...
static void sdl_joypad_destroy(void)
{
   ...
   memset(sdl3_pads, 0, sizeof(sdl3_pads));
      ...
      unsigned port, uint16_t joykey)
{
   ...
   if (hat_dir)
   {
      ...
      return sdl3_pad_get_button(pad, joykey);
      ...
      unsigned port, uint32_t joyaxis)
{
   ...
   {
      int16_t val = sdl3_pad_get_axis(pad, AXIS_NEG_GET(joyaxis));
      if (val < 0)
      {
   ...
   {
      int16_t val = sdl3_pad_get_axis(pad, AXIS_POS_GET(joyaxis));
      if (val > 0)
         return val;
   }
            ...
            SDL_EVENT_JOYSTICK_ADDED, SDL_EVENT_JOYSTICK_REMOVED) > 0)
   {
      switch (event.type)
      {
         case SDL_EVENT_JOYSTICK_ADDED:
            sdl3_pad_connect(event.jdevice.which);
            break;
         case SDL_EVENT_JOYSTICK_REMOVED:
            sdl3_pad_disconnect(event.jdevice.which);
            break;
      }
   }
...
static bool sdl_joypad_set_rumble(unsigned pad,
      enum retro_rumble_effect effect, uint16_t strength)
{
   sdl3_joypad_t *joypad = (sdl3_joypad_t*)&sdl3_pads[pad];
   uint16_t low          = 0;
   uint16_t high         = 0;
...
static bool sdl_joypad_set_sensor_state(unsigned pad,
...
{
   if (pad >= MAX_USERS || !sdl3_pads[pad].joypad)
      return false;
   if (sdl3_pads[pad].gamepad)
      return SDL_GamepadConnected(sdl3_pads[pad].gamepad);
   return true;
}
