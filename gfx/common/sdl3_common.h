...
/* Drains quit / window / display events from the SDL event queue,
 * setting *quit / *resize to true when one arrives (never back to
 * false - callers pre-initialize them). Keyboard/mouse events are
 * deliberately left in the queue for the SDL3 input driver. */
